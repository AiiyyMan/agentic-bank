...
| CB-03 | **Transaction listing tool** | Implement `get_transactions` tool. Direct Supabase query on local `transactions` table (NOT through BankingPort — see mock-strategy.md §6.1). Support filters: account_id, category, start_date, end_date, merchant (ILIKE), limit, offset. Return with total count and has_more. | `apps/api/src/tools/core-banking.ts` (add handler), `apps/api/src/tools/schemas/core-banking.ts` (add schema) | Foundation (transactions table, seed data) | M | Unit test: filter combinations return expected subsets from seeded data |
| CB-04 | **Transaction categorisation** | Create merchant-to-category map for top 50 UK merchants. Map: merchant_name → { category, category_icon }. Categories: Groceries, Dining, Transport, Entertainment, Shopping, Bills, Health, Travel, Education, Other. Normalise by prefix matching ("Tesco Express" → "Tesco" → Groceries). Export as `categorise_transaction` internal tool. | `apps/api/src/services/categorisation.ts`, `apps/api/src/tools/core-banking.ts` (register internal tool) | None | M | Unit test: 50 known merchants categorised correctly; unknown defaults to "Other" |
