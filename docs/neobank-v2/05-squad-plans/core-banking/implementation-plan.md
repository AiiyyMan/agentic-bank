...
| CB-04 | **Transaction categorisation (PFCv2 hybrid pipeline)** | Hybrid categorisation pipeline using Plaid PFCv2 taxonomy (16 primary, 111 detailed categories). (1) Merchant normalisation function: lowercase, strip LTD/PLC/& CO suffixes, collapse whitespace. (2) Rule-based map for top 50-100 UK merchants → `{ primary_category, detailed_category, category_icon }`. (3) `merchant_categories` cache table lookup. (4) Claude Haiku fallback for unknowns → write result to cache. (5) Fallback: GENERAL_MERCHANDISE / Other if Haiku unavailable. Also implements `is_recurring` detection: flag transactions from known subscription merchants (Netflix, Spotify, etc.) and detect recurring patterns (same merchant + similar amount within ±5%). `categorise_transaction` is server-side infrastructure, NOT a Claude tool. | `apps/api/src/services/categorisation.ts`, `apps/api/src/services/merchant-normaliser.ts` | Foundation (transactions table, merchant_categories table) | M | Unit test: 50+ known merchants categorised to correct PFCv2 category; unknown merchant triggers Haiku mock and caches result; normaliser handles "TESCO STORES LTD" → "tesco"; is_recurring detected for Netflix/Spotify |
...
| CB-09 | **PaymentService foundation** | Create PaymentService with constructor injection. Implement: `sendPayment(userId, params)` — validates beneficiary_id belongs to user, amount 0.01-10000, reference max 18 chars, sufficient balance. Creates pending_action. `executePayment(actionId)` — calls BankingPort.sendPayment(), inserts payment + transaction records, updates beneficiary last_used_at, writes audit_log. Define errors: `InsufficientFundsError`, `InvalidBeneficiaryError`, `PaymentLimitExceededError`. | `apps/api/src/services/payment.ts` | CB-01, CB-08, Foundation (payments table) | M | Unit test: valid payment → pending_action; insufficient funds → error; invalid beneficiary → error |
...
| CB-14 | **REST endpoints — pots** | POST /api/pots (create), POST /api/pots/:id/transfer (deposit/withdraw), PATCH /api/pots/:id (update), DELETE /api/pots/:id (close). Write endpoints route through PotService. Read: GET from BankingPort. | `apps/api/src/routes/pots.ts`, `apps/api/src/server.ts` (register plugin) | CB-06, CB-07 | M | API test: full CRUD lifecycle; write endpoints validate via PotService |
| CB-15 | **REST endpoints — beneficiaries** | GET /api/beneficiaries, POST /api/beneficiaries, DELETE /api/beneficiaries/:id. Write endpoints route through PaymentService. | `apps/api/src/routes/beneficiaries.ts`, `apps/api/src/server.ts` (register plugin) | CB-08, CB-09 | S | API test: list, add, delete lifecycle; duplicate detection |
...
| CB-17 | **Account Detail screen** | Mobile drill-down screen. Fetch account data + recent transactions via REST. Balance at top, account info with copy, transaction list (5-10), quick actions. Loading/error/empty states. | `apps/mobile/src/app/(tabs)/accounts/[id].tsx`, `apps/mobile/src/components/accounts/AccountInfo.tsx`, `apps/mobile/src/components/shared/CopyButton.tsx` | CB-12, CB-13, Foundation (mobile scaffold) | M | Visual: balance renders, copy works, transactions list, loading skeleton |
| CB-18 | **Transaction List screen** | Mobile drill-down. Fetch transactions with filters. Date-grouped sections. Category filter pills. Infinite scroll pagination. | `apps/mobile/src/app/(tabs)/accounts/[id]/transactions.tsx`, `apps/mobile/src/components/transactions/TransactionRow.tsx`, `apps/mobile/src/components/transactions/DateGroupHeader.tsx`, `apps/mobile/src/components/shared/CategoryIcon.tsx` | CB-13, Foundation (mobile scaffold) | M | Visual: date groups render, filter toggles, infinite scroll loads more |
| CB-19 | **Savings Tab screen** | Mobile tab screen. Total savings header. Pot card list (vertical FlatList). Progress bars. Create pot FAB. Empty state. | `apps/mobile/src/app/(tabs)/savings.tsx`, `apps/mobile/src/components/pots/PotListCard.tsx`, `apps/mobile/src/components/shared/ProgressBar.tsx` | CB-14, Foundation (mobile scaffold) | M | Visual: pot cards render with progress bars, totals correct, empty state shows |
| CB-20 | **Beneficiary List screen** | Mobile drill-down. Search bar. Recent/all sections. Beneficiary rows. Add payee action. | `apps/mobile/src/app/(tabs)/payments/beneficiaries.tsx`, `apps/mobile/src/components/beneficiaries/BeneficiaryRow.tsx` | CB-15, Foundation (mobile scaffold) | M | Visual: search filters, sections render, add action navigates |
...
- `categorise_transaction` — input: merchant_name → output: { primary_category, detailed_category, category_icon, is_recurring }
  > **Note:** This is a server-side internal function, NOT a Claude tool. It runs automatically when transactions are ingested. Uses PFCv2 taxonomy via hybrid pipeline (rules → merchant cache → Haiku fallback). Not registered in the tool registry and does not appear in Claude's tool list.
...
**CB-04 (Transaction categorisation) must complete by Day 5** so EX-Insights can build spending queries on categorised data. This is the only hard cross-squad deadline for CB in Phase 1.
