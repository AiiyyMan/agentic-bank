...
|---------|------|----------------|------|
| **No audit log table** | Low for POC (demo only). High if any data persists beyond demo. | **ADDRESSED:** `audit_log` table added (data-model.md §2.24). Domain services write to it on every state mutation. Added to Foundation F1b (system-architecture.md §11.4.3). Migration 017. | POC |
